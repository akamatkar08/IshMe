...
SELECT facid, name, membercost, monthlymaintenace FROM Facilities WHERE (membercost / monthlymaintenance) < .20
...
END AS cost
FROM Bookings
INNER JOIN Facilites ON Bookings.facid = Facilites.facid
AND Bookings
INNER JOIN Facilites ON Bookings.facid = Facilities.facid
...
CASE WHEN Bookings.memid = 0
THEN Facilities.guestcost * Bookings.slots
ELSE Facilites.membercost * Bookings.slots
END AS cost
FROM Bookings
INNER JOIN Facilites ON Bookings.facid = Facilites.facid
AND Bookings.starttime LIKE '2012-09-14%'
